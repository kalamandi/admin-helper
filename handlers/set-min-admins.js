exports.execute = async (ctx) => { 
    ...
    if (
        ...
        || ctx.peer.min_admins == min_admins
            ...
            return ctx.reply(ctx.peer.notify == 0 ? 
                '🔕 Уведомления при маленьком количестве админов на сервере отключены.\n\nДля включения напишите /notify' 
                : `🔔 Уведомления приходят когда на сервере меньше ${ctx.peer.min_admins} администраторов.\n\nДля изменения минимального числа администраторов, напишите /minadm [новое число]`
            );
    };

...
exports.execute = async (ctx) => {
    ...
    if (!ctx.user) return ctx.api.messages.sendMessageEventAnswer({
    ...
    if ((action == 'out' && !isOnServer) 
    || (action == 'in' && isOnServer)) return ctx.api.messages.sendMessageEventAnswer({
        event_id: ctx.eventId,
        peer_id: ctx.peerId,
        user_id: ctx.userId,
        event_data: JSON.stringify({
    ...
    if (action == 'in') await showAdmins(ctx, 1);
    else await ctx.api.messages.send({
